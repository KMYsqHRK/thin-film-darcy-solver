                           ...
                           const std::string& output_dir_override)
{
    ...
    try {
        ...
        // Directory settings
        if (config.contains("input_dir")) {
            m_settings.input_dir = config["input_dir"];
        }
        if (config.contains("output_dir")) {
            m_settings.output_dir = config["output_dir"];
        }
        ...
        // Boundary condition types for edges
        if (config.contains("boundary_conditions")) {
            m_settings.boundary_edges.clear();
            const auto& bc_config = config["boundary_conditions"];

            // Define edge names
            std::vector<std::string> edge_names = {"bottom", "right", "top", "left"};

            for (int edge_idx = 0; edge_idx < 4; ++edge_idx) {
                BoundaryEdge edge;
                edge.edge_index = edge_idx;

                // Default to Dirichlet if not specified
                edge.type = BoundaryConditionType::DIRICHLET;

                if (bc_config.contains(edge_names[edge_idx])) {
                    std::string bc_type_str = bc_config[edge_names[edge_idx]].value("type", "dirichlet");

                    // Convert to lowercase for comparison
                    std::transform(bc_type_str.begin(), bc_type_str.end(), bc_type_str.begin(), ::tolower);

                    if (bc_type_str == "neumann") {
                        edge.type = BoundaryConditionType::NEUMANN;
                    } else if (bc_type_str == "dirichlet") {
                        edge.type = BoundaryConditionType::DIRICHLET;
                    } else {
                        std::cerr << "Warning: Unknown boundary condition type '" << bc_type_str
                                  << "' for edge " << edge_names[edge_idx] << ". Using Dirichlet." << std::endl;
                    }
                }

                m_settings.boundary_edges.push_back(edge);
            }
        } else {
            // Default: all edges use Dirichlet boundary conditions (backward compatibility)
            m_settings.boundary_edges.clear();
            for (int edge_idx = 0; edge_idx < 4; ++edge_idx) {
                BoundaryEdge edge;
                edge.edge_index = edge_idx;
        ...
        if (!input_dir_override.empty())  m_settings.input_dir  = input_dir_override;
        if (!output_dir_override.empty()) m_settings.output_dir = output_dir_override;

        // Resolve file paths with input_dir / output_dir
        auto join_path = [](const std::string& dir, const std::string& file) -> std::string {
            if (dir.empty()) return file;
            return dir + "/" + file;
        };
        m_settings.input_pressure_file    = join_path(m_settings.input_dir,  m_settings.input_pressure_file);
        m_settings.output_lift_force_file = join_path(m_settings.output_dir, m_settings.output_lift_force_file);
        m_settings.output_pressure_grid_file = join_path(m_settings.output_dir, m_settings.output_pressure_grid_file);

        // Create output directory if it does not exist
        if (!m_settings.output_dir.empty()) {
            fs::create_directories(m_settings.output_dir);
        }

        std::cout << "Settings loaded successfully:" << std::endl;
        std::cout << "  Input dir:  " << (m_settings.input_dir.empty() ? "(cwd)" : m_settings.input_dir) << std::endl;
        std::cout << "  Output dir: " << m_settings.output_dir << std::endl;
        ...
        std::vector<std::string> edge_names = {"bottom", "right", "top", "left"};
        for (size_t i = 0; i < m_settings.boundary_edges.size(); ++i) {
            std::string bc_type = (m_settings.boundary_edges[i].type == BoundaryConditionType::DIRICHLET)
                                  ? "Dirichlet" : "Neumann";
            std::cout << "    " << edge_names[i] << ": " << bc_type << std::endl;
        }
...
// Load input data using path resolved from settings
void Solver::load_input_data()
{
    load_input_data(m_settings.input_pressure_file);
}
...
void Solver::load_input_data(const std::string& filename)
{
    ...
    // Determine which edges are Dirichlet (in order: 0=bottom,1=right,2=top,3=left)
    // The CSV contains values only for Dirichlet edges; Neumann edges default to 0.
    int n = m_settings.division_number;
    std::array<bool, 4> is_dirichlet = {false, false, false, false};
    for (const auto& edge : m_settings.boundary_edges) {
        if (edge.edge_index >= 0 && edge.edge_index < 4) {
            is_dirichlet[edge.edge_index] = (edge.type == BoundaryConditionType::DIRICHLET);
        }
    }
    int dirichlet_edge_count = 0;
    for (bool d : is_dirichlet) { if (d) dirichlet_edge_count++; }
    int expected_pressure_count = dirichlet_edge_count * n;
    ...
    while (std::getline(file, line)) {
        ...
        // Read boundary values (Dirichlet edges only)
        std::vector<double> csv_values;
        while (std::getline(iss, value, ',')) {
            csv_values.push_back(std::stod(value));
        }

        // Validate boundary value count
        if (csv_values.size() != static_cast<size_t>(expected_pressure_count)) {
            std::cerr << "Warning: Line " << line_number << " has " << csv_values.size()
                      << " boundary values, expected " << expected_pressure_count
                      << " (Dirichlet edges only)" << std::endl;
            continue;
        }

        // Expand to 4n: Dirichlet edges use CSV values, Neumann edges filled with 0
        data.boundary_values.assign(4 * n, 0.0);
        int csv_idx = 0;
        for (int edge = 0; edge < 4; edge++) {
            if (is_dirichlet[edge]) {
                for (int k = 0; k < n; k++) {
                    data.boundary_values[edge * n + k] = csv_values[csv_idx++];
                }
            }
        }
...
// Get boundary condition type for a given grid position
BoundaryConditionType Solver::get_boundary_condition_type(int i, int j, int n) const
{
    // Check if this point is on a boundary
    if (i == 0) {
        // Bottom edge
        return m_settings.boundary_edges.size() > 0 ? m_settings.boundary_edges[0].type : BoundaryConditionType::DIRICHLET;
    } else if (j == n) {
        // Right edge
        return m_settings.boundary_edges.size() > 1 ? m_settings.boundary_edges[1].type : BoundaryConditionType::DIRICHLET;
    } else if (i == n) {
        // Top edge
        return m_settings.boundary_edges.size() > 2 ? m_settings.boundary_edges[2].type : BoundaryConditionType::DIRICHLET;
    } else if (j == 0) {
        // Left edge
        return m_settings.boundary_edges.size() > 3 ? m_settings.boundary_edges[3].type : BoundaryConditionType::DIRICHLET;
    }

    // Interior point - not a boundary
    return BoundaryConditionType::DIRICHLET;  // Default, though this shouldn't be called for interior points
}
...
// For Dirichlet BC: stores the pressure value
// For Neumann BC: stores 0.0 (will be solved for)
Eigen::MatrixXd Solver::create_pressure_matrix(const std::vector<double>& boundary_values, int n)
{
    Eigen::MatrixXd pressure_matrix = Eigen::MatrixXd::Zero(n + 1, n + 1);

    if (boundary_values.size() != static_cast<size_t>(4 * n)) {
        std::cerr << "Error: Expected " << 4 * n << " boundary values, got "
                  << boundary_values.size() << std::endl;
        return pressure_matrix;
    }

    // 境界値を行列の境界要素に配置
    // For Neumann boundaries, the values represent gradients (not pressures)
    // and will be used in the RHS construction
    int idx = 0;

    // 下辺 (i=0, j=0 to n-1) - edge index 0
    BoundaryConditionType bc_bottom = get_boundary_condition_type(0, 0, n);
    for (int j = 0; j < n; j++) {
        if (bc_bottom == BoundaryConditionType::DIRICHLET) {
            pressure_matrix(0, j) = boundary_values[idx];
        } else {
            // Neumann BC: initialize to 0, will be solved for
            pressure_matrix(0, j) = 0.0;
        }
        idx++;
    }

    // 右辺 (i=0 to n-1, j=n) - edge index 1
    BoundaryConditionType bc_right = get_boundary_condition_type(0, n, n);
    for (int i = 0; i < n; i++) {
        if (bc_right == BoundaryConditionType::DIRICHLET) {
            pressure_matrix(i, n) = boundary_values[idx];
        } else {
            pressure_matrix(i, n) = 0.0;
        }
        idx++;
    }

    // 上辺 (i=n, j=n to 1) - edge index 2
    BoundaryConditionType bc_top = get_boundary_condition_type(n, n, n);
    for (int j = n; j > 0; j--) {
        if (bc_top == BoundaryConditionType::DIRICHLET) {
            pressure_matrix(n, j) = boundary_values[idx];
        } else {
            pressure_matrix(n, j) = 0.0;
        }
        idx++;
    }

    // 左辺 (i=n to 1, j=0) - edge index 3
    BoundaryConditionType bc_left = get_boundary_condition_type(n, 0, n);
    for (int i = n; i > 0; i--) {
        if (bc_left == BoundaryConditionType::DIRICHLET) {
            pressure_matrix(i, 0) = boundary_values[idx];
        } else {
            pressure_matrix(i, 0) = 0.0;
        }
        idx++;
    }
...
void Solver::update_wet_dry_matrix(const std::vector<double>& /*boundary_values*/)
{
...
void Solver::process_single_timestep(const TimestepData& data, int timestep_index)
{
    ...
    // Store current boundary values for use in RHS construction
    current_boundary_values = data.boundary_values;

    // Create pressure matrix from boundary values (Dirichlet) or initialize for Neumann
    boundary_conditions = create_pressure_matrix(data.boundary_values, n);
    ...
    if (has_pressure && has_dry_points) {
        update_wet_dry_matrix(data.boundary_values);
    }
...
// Also handles Neumann boundary conditions
                                                               ...
                                                               const Eigen::MatrixXi& grid_to_compressed)
{
    ...
    for (int i = 0; i <= n; ++i) {
        for (int j = 0; j <= n; ++j) {
            // Skip dry points and Dirichlet boundary points
            if (grid_to_compressed(i, j) < 0) continue;

            int row = grid_to_compressed(i, j);

            // Check if this is a Neumann boundary point
            bool is_boundary = (i == 0 || i == n || j == 0 || j == n);
            if (is_boundary) {
                BoundaryConditionType bc_type = get_boundary_condition_type(i, j, n);

                if (bc_type == BoundaryConditionType::NEUMANN) {
                    // Neumann BC: ∂P/∂n = g
                    // Use first-order one-sided finite difference

                    if (i == 0) {
                        // Bottom edge: ∂P/∂y = (P(1,j) - P(0,j)) / dy
                        triplets.emplace_back(row, row, -1.0 / dy);  // Coefficient for P(0,j)
                        if (grid_to_compressed(1, j) >= 0) {
                            triplets.emplace_back(row, grid_to_compressed(1, j), 1.0 / dy);  // P(1,j)
                        }
                    } else if (i == n) {
                        // Top edge: ∂P/∂y = (P(n,j) - P(n-1,j)) / dy
                        triplets.emplace_back(row, row, 1.0 / dy);  // Coefficient for P(n,j)
                        if (grid_to_compressed(n-1, j) >= 0) {
                            triplets.emplace_back(row, grid_to_compressed(n-1, j), -1.0 / dy);  // P(n-1,j)
                        }
                    } else if (j == 0) {
                        // Left edge: ∂P/∂x = (P(i,1) - P(i,0)) / dx
                        triplets.emplace_back(row, row, -1.0 / dx);  // Coefficient for P(i,0)
                        if (grid_to_compressed(i, 1) >= 0) {
                            triplets.emplace_back(row, grid_to_compressed(i, 1), 1.0 / dx);  // P(i,1)
                        }
                    } else if (j == n) {
                        // Right edge: ∂P/∂x = (P(i,n) - P(i,n-1)) / dx
                        triplets.emplace_back(row, row, 1.0 / dx);  // Coefficient for P(i,n)
                        if (grid_to_compressed(i, n-1) >= 0) {
                            triplets.emplace_back(row, grid_to_compressed(i, n-1), -1.0 / dx);  // P(i,n-1)
                        }
                    }
                    continue;  // Skip standard interior stencil for Neumann boundaries
                } else {
                    // Dirichlet boundary - skip (not in compressed system)
                    continue;
                }
            }
    ...
    std::cout << "Compressed coefficient matrix created: " << wet_point_count << "x" << wet_point_count
              << " system with " << triplets.size() << " non-zero entries (including Neumann BCs)" << std::endl;
                                             ...
                                             const std::vector<double>& boundary_values)
{
    ...
    auto get_boundary_value = [&](int i, int j, int n) -> double {
        // Map (i,j) to index in boundary_values vector
        // Edge order: bottom (0), right (1), top (2), left (3)
        if (i == 0 && j < n) {
            // Bottom edge
            return boundary_values[j];
        } else if (j == n && i < n) {
            // Right edge
            return boundary_values[n + i];
        } else if (i == n && j > 0) {
            // Top edge (reversed)
            return boundary_values[2*n + (n - j)];
        } else if (j == 0 && i > 0) {
            // Left edge (reversed)
            return boundary_values[3*n + (n - i)];
        }
    ...
    for (int i = 0; i <= n; ++i) {
        for (int j = 0; j <= n; ++j) {
            ...
            if (is_boundary) {
                ...
                if (bc_type == BoundaryConditionType::NEUMANN) {
                    // For Neumann BC, RHS contains the specified gradient value
                    double gradient = get_boundary_value(i, j, n);
                    rhs(idx) = gradient;
                    continue;  // Skip interior stencil for Neumann boundaries
                } else {
                    // Dirichlet boundary - skip (should not be in compressed system)
                    continue;
                }
            }
...
Eigen::MatrixXd Solver::solve_pressure_distribution(const Eigen::MatrixXd& boundary_matrix, int n)
{
    ...
    // First pass: assign indices to wet interior points AND Neumann boundary points
    for (int i = 0; i <= n; ++i) {
        for (int j = 0; j <= n; ++j) {
            // Include point if it's:
            // 1. A wet interior point, OR
            // 2. A boundary point with Neumann BC
            bool is_interior_wet = (i > 0 && i < n && j > 0 && j < n && m_wet_dry_matrix(i, j) > 0.5);
            bool is_neumann_boundary = (i == 0 || i == n || j == 0 || j == n) &&
                                       (get_boundary_condition_type(i, j, n) == BoundaryConditionType::NEUMANN);

            if (is_interior_wet || is_neumann_boundary) {
    ...
    // Step 3: Construct compressed RHS vector with P=0 at wet-dry interface and Neumann gradients
    Eigen::VectorXd rhs = construct_rhs_vector(boundary_matrix, n, m_wet_dry_matrix, grid_to_compressed, current_boundary_values);
    ...
    std::cout << "Pressure distribution solved successfully with wet-dry treatment and mixed BCs" << std::endl;
    std::cout << "==============================================================================\n" << std::endl;
