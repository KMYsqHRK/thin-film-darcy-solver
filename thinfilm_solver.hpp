...
enum class BoundaryConditionType {
    DIRICHLET,  // Specified pressure value
    NEUMANN     // Specified pressure gradient (flux)
};
...
// Boundary edge configuration (one for each of 4 edges)
struct BoundaryEdge {
    BoundaryConditionType type;
    int edge_index;  // 0=bottom, 1=right, 2=top, 3=left
};

// Time-series data for a single timestep
struct TimestepData {
    double time;
    std::vector<double> boundary_values;  // 4n boundary values (pressure or gradient depending on BC type)
};

// 設定データ
struct Settings {
    // Directories (relative to working directory)
    std::string input_dir;                   // Directory containing input CSV file
    std::string output_dir;                  // Directory for output files (created if absent)

    // Input/Output files (resolved with input_dir / output_dir)
    ...
    std::vector<BoundaryEdge> boundary_edges;  // Configuration for each edge (4 edges)
...
public:
    ...
    std::vector<double> current_boundary_values;  // Raw boundary values for current timestep
    ...
    void load_input_data();  // uses path resolved from settings (input_dir + input_pressure_file)
    ...
    Eigen::MatrixXd create_pressure_matrix(const std::vector<double>& boundary_values, int n);
    BoundaryConditionType get_boundary_condition_type(int i, int j, int n) const;
